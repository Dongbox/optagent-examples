...
`solve(...)`，由求解器根据结构能力自动编排搜索组合；简单的线性、路径和调度问题不应
为了展示策略而改写成黑盒。
<!-- --8<-- [end:problem] -->
...
钢卷顺序的焊接兼容性依赖业务规则，目标值由 Python 函数逐个评价相邻钢卷
得到。规则可以继续扩展，但无法稳定地展开为线性或 CP-SAT 表达式，因此
使用 `external_call` 注册纯、确定且可缓存的外部评估器，再调用通用 `solve`。

只有这类确实依赖外部模拟器、遗留业务函数或远程服务的问题才推荐黑盒路径；
简单的 TSP、线性路径和固定调度应优先采用结构化建模。
<!-- --8<-- [end:solve_case] -->
